...
for e_id in tqdm(unread_msg_nums):
    ...
    tzone_delta = round((tzone[9] - 3*3600)/3600)       #сравнивается текущая зона с зоной в кортеже (10800 это +3000) (вынести текущий часовой пояс "3" в конфиг)
    ...
    if tzone_delta != 0:                                                                    # если разница часовых зон не равно 0
        ...
        if d_fact_str != '':                                                                # если письмо фактически было прислано вчера требуется уменьшить день
